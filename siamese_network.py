...
class SiameseLSTM(object):
    ...
    def BiRNN(self, x, dropout, scope, embedding_size, sequence_length):
        ...
        n_hidden=n_steps
        n_layers=3
        # Prepare data shape to match `bidirectional_dynamic_rnn` function requirements
        ...
        with tf.name_scope("bw"+scope),tf.variable_scope("bw"+scope):
            ...
            #             outputs = tf.contrib.rnn.static_bidirectional_rnn(lstm_fw_cell_m, lstm_bw_cell_m, x,
            #                                             dtype=tf.float32)
        return outputs[-1]
